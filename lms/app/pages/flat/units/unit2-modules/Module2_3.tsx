...
const Module2_3: React.FC = () => {
    ...
    const handleQuizSelect = (qId: number, oIdx: number) => {
    ...
    const checkQuiz = () => {
        const correctAnswers: Record<number, number> = {
            1: 1, // B - number of states
            2: 2, // C - i = m+1
            3: 2, // C - aⁿbᵐ
            4: 1, // B - |xy| ≤ p
            5: 2, // C - necessary and sufficient
            6: 1, // B - {0ⁱ}
            8: 1, // False (False - two non-regular can have regular union)
        };
        const results: Record<number, boolean> = {};
        Object.keys(correctAnswers).forEach(qId => {
            const id = parseInt(qId);
            results[id] = quizAnswers[id] === correctAnswers[id];
        });
    ...
    const mcqQuestions = [
        {
            id: "mcq-1",
            q: "The Pumping Lemma for regular languages states that if L is regular, there exists p ≥ 1 such that for every s ∈ L with |s| ≥ p, we can write s = xyz. Which combination of conditions must ALL hold simultaneously?",
            options: [
                "|y| ≥ 1, |xyz| ≤ p, and xyⁱz ∈ L for all i ≥ 0",
                "|y| ≥ 0, |xy| ≤ p, and xyⁱz ∈ L for all i ≥ 1",
                "|y| ≥ 1, |xy| ≤ p, and xyⁱz ∈ L for all i ≥ 0",
                "|y| ≥ 1, |yz| ≤ p, and xyⁱz ∈ L for all i ≥ 0"
            ],
            correct: 2
        },
        {
            id: "mcq-2",
            q: "You want to prove L = {aⁿbⁿ | n ≥ 0} is not regular. You choose s = aᵖbᵖ. The adversary splits s = xyz with |xy| ≤ p. What does condition |xy| ≤ p guarantee about the structure of y?",
            options: [
                "y consists entirely of b's, because |xy| ≤ p and there are p b's at the end",
                "y consists entirely of a's, because xy lies within the first p characters which are all a's",
                "y must contain both a's and b's, since |xy| ≤ p and the string has both",
                "y can be any substring of s as long as |y| ≥ 1"
            ],
            correct: 1
        }
    ];
...
5. CONTRADICTION: Condition (3) fails. 
   Therefore, L is NOT regular. □`}
            ...
            {/* Interactive MCQs */}
            <section className="mb-24">
                <div className="bg-indigo-50 rounded-[4rem] p-16 text-slate-900 shadow-xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-full h-full bg-indigo-500/5 -skew-x-12 translate-x-1/2"></div>
                    <div className="relative z-10">
                        <div className="text-center mb-16">
                            <h2 className="text-5xl font-black uppercase tracking-tighter mb-4 text-indigo-900 italic font-serif">Quick Assessment</h2>
                            <div className="h-1.5 w-32 bg-indigo-600 mx-auto rounded-full mb-6 shadow-lg shadow-indigo-100"></div>
                            <p className="text-xs text-indigo-400 font-black uppercase tracking-[0.4em]">Test your adversarial logic</p>
                        </div>

                        <div className="max-w-4xl mx-auto space-y-10">
                            {mcqQuestions.map((q, idx) => (
                                <div key={idx} className="bg-white border-2 border-indigo-100 p-10 rounded-[3rem] group hover:bg-slate-50/50 transition-all shadow-sm">
                                    <div className="flex justify-between items-start mb-8">
                                        <div className="bg-indigo-600 text-white text-[10px] font-black px-4 py-1 rounded-full uppercase italic tracking-widest">Question {idx + 1}</div>
                                        <GraduationCap className="w-8 h-8 text-indigo-100 group-hover:text-indigo-600 transition-colors" />
                                    </div>
                                    <p className="text-xl font-bold mb-10 group-hover:text-indigo-900 transition-colors leading-relaxed text-slate-800 italic font-serif">{q.q}</p>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {q.options.map((opt, i) => (
                                            <button
                                                key={i}
                                                onClick={() => handleQuizSelect(idx + 1, i)}
                                                className={`text-left p-5 rounded-2xl border-2 transition-all font-bold text-xs uppercase tracking-tight ${quizAnswers[idx + 1] === i ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl scale-[1.02] italic' : 'bg-white border-slate-100 hover:border-indigo-200 hover:bg-indigo-50'}`}
                                            >
...
#    witness = verify_pumping(lang, x, y, z, p)`}
            ...
            <section className="mb-24">
                <div className="bg-white border-4 border-indigo-600 rounded-[4rem] p-16 shadow-2xl relative overflow-hidden group">
                    <div className="absolute inset-x-0 top-0 h-4 bg-indigo-600"></div>
                    <div className="relative z-10 flex flex-col items-center">
                        <div className="bg-indigo-600 text-white px-10 py-2 rounded-full text-xs font-black uppercase tracking-[0.5em] mb-12 shadow-xl italic">Section Final Quiz</div>
                        <h2 className="text-6xl font-black italic uppercase tracking-tighter text-slate-900 text-center mb-16 leading-[0.9]">PROVE YOUR <br />THEORETICAL <span className="text-indigo-600">MIGHT</span></h2>

                        <div className="w-full max-w-4xl space-y-12">
                            {[
                                { id: 1, q: "What is the pumping length p intuitively equal to in a DFA?", o: ["Shortest string length", "Number of states in minimal DFA", "Alphabet size", "Diameter of the graph"], c: 1 },
                                { id: 2, q: "For L = {aᵖ | p is prime}, which i value breaks the prime length for s=aᵐ?", o: ["i=0", "i=2", "i=m+1", "i=1"], c: 2 },
                                { id: 3, q: "Which of these is NOT regular?", o: ["(a+b)*", "a*b*", "{aⁿbᵐ | n,m ≥ 1}", "{aⁿbⁿ | n ≥ 0}"], c: 3 },
                                { id: 5, q: "Myhill-Nerode gives what kind of condition for regularity?", o: ["Necessary only", "Sufficient only", "Necessary and Sufficient", "Approximate"], c: 2 }
                            ].map((q, idx) => (
                                <div key={q.id} className="group/q bg-slate-50 p-10 rounded-[2.5rem] border-2 border-slate-100 hover:border-indigo-200 transition-all shadow-sm">
                                    <p className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest tracking-[0.4em]">ASSESSMENT 2.3.{q.id}</p>
                                    <p className="text-2xl font-black mb-10 group-hover/q:text-indigo-900 transition-colors leading-relaxed font-serif italic text-slate-800">{q.q}</p>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {q.o.map((opt, i) => (
                                            <button
                                                key={i}
                                                onClick={() => handleQuizSelect(q.id, i)}
                                                className={`text-left p-6 rounded-2xl border-2 transition-all text-xs font-bold uppercase tracking-tight ${quizAnswers[q.id] === i ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl' : 'bg-white border-slate-200 hover:bg-indigo-50 hover:border-indigo-300'}`}
                                            >
                                                {opt}
                                            </button>
                                        ))}
                                    ...
                                    {quizSubmitted && (
                                        <div className={`mt-8 p-6 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] italic border-2 ${quizResults[q.id] ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-rose-50 border-rose-100 text-rose-600 shadow-sm shadow-rose-100'}`}>
                                            {quizResults[q.id] ? "✨ THEOREM VALIDATED: MATCHED INFRASTRUCTURE" : "❌ COLLISION DETECTED: REVIEW STATE MAPPING"}
                                        </div>
                                    )}
                                </div>
                            ))}
                            <div className="pt-8 flex justify-center">
                                <button
                                    onClick={checkQuiz}
                                    className="bg-indigo-600 text-white px-20 py-6 rounded-full text-sm font-black uppercase tracking-[0.5em] shadow-2xl hover:bg-indigo-700 transition-all hover:scale-[1.05] italic"
                                >
                                    {quizSubmitted ? "RETRY ASSESSMENT" : "SUBMIT FOR GRADING"}
                                </button>
                            </div>
                        </div>
                    </div>
