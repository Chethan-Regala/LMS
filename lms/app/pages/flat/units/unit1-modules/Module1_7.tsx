...
const Module1_7: React.FC = () => {
  ...
  const [mcqAnswers, setMcqAnswers] = useState<{[key: string]: boolean}>({});
...
print("Non-greedy:", non_greedy)`}
      ...
      <section style={{ marginBottom: '48px' }}>
        ...
        <p style={{ marginBottom: '24px', color: '#6b7280' }}>Test your understanding of regular expressions with these practice questions. Select an answer and click "Check Answer" for instant feedback with explanations.</p>

        {/* Q1 */}
        <div style={{ backgroundColor: '#f9fafb', border: '2px solid #d1d5db', borderRadius: '8px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px' }}>Q1. Which of the following strings is NOT in the language described by the regular expression a(b + c)*d?</h3>
          <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
            {[
              { id: 'q1a', text: '"ad"', correct: false },
              { id: 'q1b', text: '"abcd"', correct: false },
              { id: 'q1c', text: '"abd"', correct: false },
              { id: 'q1d', text: '"aad"', correct: true }
            ].map(option => (
              <label key={option.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '6px', cursor: 'pointer', border: '2px solid #e5e7eb' }}>
                <input type="radio" name="q1" value={option.id} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '0.95rem', color: '#374151' }}>{option.text}</span>
              </label>
            ))}
          </div>
          <button onClick={() => setMcqAnswers({...mcqAnswers, q1: true})} style={{ backgroundColor: '#3b82f6', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          {mcqAnswers.q1 && (
            <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dcfce7', border: '2px solid #10b981', borderRadius: '6px' }}>
              <div style={{ fontWeight: '700', color: '#065f46', marginBottom: '8px' }}>✅ Correct Answer: "aad"</div>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#047857' }}>The regex a(b + c)*d requires: 'a' at start, then zero or more 'b' or 'c', then 'd' at end. "aad" has two 'a's at the start, which violates the pattern. The others match: "ad" (zero b/c), "abcd" (b then c), "abd" (just b).</p>
            </div>
          )}
        </div>
        ...
        {/* Q2 */}
        <div style={{ backgroundColor: '#f9fafb', border: '2px solid #d1d5db', borderRadius: '8px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px' }}>Q2. The regular expression (0 + 1)*1(0 + 1)* over {'{0,1}'} describes the set of all binary strings that:</h3>
          <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
            {[
              { id: 'q2a', text: 'End with 1', correct: false },
              { id: 'q2b', text: 'Start with 1', correct: false },
              { id: 'q2c', text: 'Contain at least one 1', correct: true },
              { id: 'q2d', text: 'Contain exactly one 1', correct: false }
            ].map(option => (
              <label key={option.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '6px', cursor: 'pointer', border: '2px solid #e5e7eb' }}>
                <input type="radio" name="q2" value={option.id} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '0.95rem', color: '#374151' }}>{option.text}</span>
              </label>
            ))}
          </div>
          <button onClick={() => setMcqAnswers({...mcqAnswers, q2: true})} style={{ backgroundColor: '#3b82f6', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          {mcqAnswers.q2 && (
            <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dcfce7', border: '2px solid #10b981', borderRadius: '6px' }}>
              <div style={{ fontWeight: '700', color: '#065f46', marginBottom: '8px' }}>✅ Correct Answer: Contain at least one 1</div>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#047857' }}>The pattern (0 + 1)* allows any prefix, then requires a mandatory '1', then (0 + 1)* allows any suffix. This ensures at least one '1' appears somewhere in the string. It doesn't restrict position (start/end) or count (can have multiple 1's).</p>
            </div>
          )}
        </div>
        ...
        {/* Q3 */}
        <div style={{ backgroundColor: '#f9fafb', border: '2px solid #d1d5db', borderRadius: '8px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px' }}>Q3. Which regular expression describes the set of all strings over {'{a, b}'} that have even length?</h3>
          <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
            {[
              { id: 'q3a', text: '(ab)*', correct: false },
              { id: 'q3b', text: '((a+b)(a+b))*', correct: true },
              { id: 'q3c', text: '(a+b)*(a+b)', correct: false },
              { id: 'q3d', text: '(aa + bb + ab + ba)*', correct: false }
            ].map(option => (
              <label key={option.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '6px', cursor: 'pointer', border: '2px solid #e5e7eb' }}>
                <input type="radio" name="q3" value={option.id} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '0.95rem', color: '#374151', fontFamily: 'monospace' }}>{option.text}</span>
              </label>
            ))}
          </div>
          <button onClick={() => setMcqAnswers({...mcqAnswers, q3: true})} style={{ backgroundColor: '#3b82f6', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          {mcqAnswers.q3 && (
            <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dcfce7', border: '2px solid #10b981', borderRadius: '6px' }}>
              <div style={{ fontWeight: '700', color: '#065f46', marginBottom: '8px' }}>✅ Correct Answer: ((a+b)(a+b))*</div>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#047857' }}>Even length means pairs of characters. (a+b)(a+b) matches any 2-character string, and the outer * repeats this zero or more times, giving strings of length 0, 2, 4, 6, etc. Option (ab)* only matches "ab" repeated, missing strings like "aa" or "ba". Option (aa + bb + ab + ba)* is equivalent but unnecessarily verbose.</p>
            </div>
          )}
        </div>
        ...
        {/* Q4 */}
        <div style={{ backgroundColor: '#f9fafb', border: '2px solid #d1d5db', borderRadius: '8px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px' }}>Q4. According to Thompson's construction, the NFA for a regular expression R* (Kleene star) requires adding:</h3>
          <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
            {[
              { id: 'q4a', text: 'Only a new start state with an ε-transition to the NFA for R', correct: false },
              { id: 'q4b', text: 'New start and accept states, with ε-transitions allowing both skipping R and looping back', correct: true },
              { id: 'q4c', text: 'A direct loop from the accept state of R back to itself', correct: false },
              { id: 'q4d', text: 'No new states; just a back-edge on the existing accept state', correct: false }
            ].map(option => (
              <label key={option.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '6px', cursor: 'pointer', border: '2px solid #e5e7eb' }}>
                <input type="radio" name="q4" value={option.id} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '0.95rem', color: '#374151' }}>{option.text}</span>
              </label>
            ))}
          </div>
          <button onClick={() => setMcqAnswers({...mcqAnswers, q4: true})} style={{ backgroundColor: '#3b82f6', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          {mcqAnswers.q4 && (
            <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dcfce7', border: '2px solid #10b981', borderRadius: '6px' }}>
              <div style={{ fontWeight: '700', color: '#065f46', marginBottom: '8px' }}>✅ Correct Answer: New start and accept states, with ε-transitions allowing both skipping R and looping back</div>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#047857' }}>Thompson's construction for R* adds: (1) new_start --ε--&gt; R_start, (2) new_start --ε--&gt; new_accept (to accept ε), (3) R_accept --ε--&gt; new_accept, (4) R_accept --ε--&gt; R_start (loop). This allows zero repetitions (skip) and multiple repetitions (loop).</p>
            </div>
          )}
        </div>
        ...
        {/* Q5 */}
        <div style={{ backgroundColor: '#f9fafb', border: '2px solid #d1d5db', borderRadius: '8px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: '700', color: '#1f2937', marginBottom: '16px' }}>Q5. Arden's Lemma states that if L = PL + Q (with ε ∉ L(P)), then the unique solution is:</h3>
          <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
            {[
              { id: 'q5a', text: 'L = P*Q*', correct: false },
              { id: 'q5b', text: 'L = PQ*', correct: false },
              { id: 'q5c', text: 'L = P*Q', correct: true },
              { id: 'q5d', text: 'L = (PQ)*', correct: false }
            ].map(option => (
              <label key={option.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '6px', cursor: 'pointer', border: '2px solid #e5e7eb' }}>
                <input type="radio" name="q5" value={option.id} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '0.95rem', color: '#374151', fontFamily: 'monospace' }}>{option.text}</span>
              </label>
            ))}
          </div>
          <button onClick={() => setMcqAnswers({...mcqAnswers, q5: true})} style={{ backgroundColor: '#3b82f6', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>Check Answer</button>
          {mcqAnswers.q5 && (
            <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dcfce7', border: '2px solid #10b981', borderRadius: '6px' }}>
              <div style={{ fontWeight: '700', color: '#065f46', marginBottom: '8px' }}>✅ Correct Answer: L = P*Q</div>
              <p style={{ margin: 0, fontSize: '0.95rem', color: '#047857' }}>Arden's Lemma: if L = PL + Q and ε ∉ L(P), then L = P*Q. This is analogous to solving X = aX + b → X = a*b in algebra. The condition ε ∉ L(P) ensures uniqueness. This is fundamental for converting automata to regular expressions.</p>
            </div>
          )}
        </div>
      </section>
