...
const deadlockHandlingQuiz = [
  {
    question: "Which deadlock handling strategy focuses on ensuring that at least one of the four Coffman conditions cannot occur?",
    options: ["Avoidance", "Detection", "Prevention", "Ignoring"],
    correctAnswer: 2,
    explanation: "Prevention works by fundamentally changing rules (like requiring resource ordering) so that a deadlock is mathematically impossible."
  },
  {
    question: "In Deadlock Avoidance, what is a 'Safe State'?",
    options: ["A state where no more resources can be allocated.", "A state where there is at least one sequence of process executions that allows everyone to finish without deadlocking.", "A state where all processes are finished.", "A state where the CPU is idle."],
    correctAnswer: 1,
    explanation: "A safe state means the OS has a 'plan' to satisfy all processes' maximum needs, even if they don't request them all at once."
  },
  {
    question: "What is the primary goal of any Deadlock Recovery method?",
    options: ["To prevent deadlocks from ever happening again.", "To break the circular wait and return the system to a functioning state after a deadlock is detected.", "To delete all files on the hard drive.", "To speed up the internet connection."],
    correctAnswer: 1,
    explanation: "Recovery assumes a deadlock has already happened; the goal is to fix it by terminating processes, preempting resources, or rolling back."
  },
  {
    question: "If an OS forcibly takes a resource away from a process to give it to another, which recovery technique is being used?",
    options: ["Process Termination", "Resource Preemption", "Checkpointing", "Mutual Exclusion"],
    correctAnswer: 1,
    explanation: "Resource Preemption involves 'stealing' a resource from a 'victim' process to allow another process to break a cycle."
  },
  {
    question: "Why would an Operating System choose 'Ignoring Deadlocks' as a strategy?",
    options: ["Because deadlocks are a myth.", "Because the programmer was lazy.", "Because in many systems, deadlocks are rare, and the cost of prevention or avoidance is higher than the cost of a rare manual restart.", "To make the system more secure."],
    correctAnswer: 2,
    explanation: "This is often called the 'Ostrich Algorithm'. It prioritizes performance and simplicity in systems where deadlocks are not a daily threat."
  }
];
  ...
  return (
    <div className="module-content">
      ...
      <section className="content-section">
        ...
        <Quiz
          title="Module 3.11 Quiz: Deadlock Handling & Recovery"
          questions={deadlockHandlingQuiz}
          subject="OS"
          unitId={3}
          moduleId={10}
        />
