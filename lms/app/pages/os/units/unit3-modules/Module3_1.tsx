...
const Module3_1: React.FC = () => {
  const syncQuiz = [
    {
      question: "Which term describes a situation where multiple processes access and manipulate shared data concurrently, and the outcome depends on the execution order?",
      options: ["Context Switch", "Race Condition", "Deadlock", "Starvation"],
      correctAnswer: 1,
      explanation: "A race condition occurs when the final result of a process depends on the specific, unpredictable sequence of execution between concurrent tasks."
    },
    {
      question: "Which requirement for solving the critical-section problem ensures that only one process is executing in its critical section at a time?",
      options: ["Mutual Exclusion", "Progress", "Bounded Waiting", "Priority Inversion"],
      correctAnswer: 0,
      explanation: "Mutual Exclusion (Mutex) is the fundamental rule that prevents multiple processes from entering their critical sections simultaneously."
    },
    {
      question: "What does the wait() (or P()) operation do in a Semaphore mechanism?",
      options: ["Increments the counter and wakes up processes.", "Decrements the counter; if the value becomes negative, the process blocks.", "Terminates the calling process.", "Deletes the shared memory space."],
      correctAnswer: 1,
      explanation: "The wait() operation effectively 'claims' a resource by decrementing the semaphore counter; if no resources are left, the caller must wait."
    },
    {
      question: "What is a 'Monitor' in the context of process synchronization?",
      options: ["A hardware device for tracking CPU usage.", "A low-level assembly instruction.", "A high-level synchronization construct that encapsulates shared variables and methods with automatic locking.", "An OS tool for terminating unresponsive processes."],
      correctAnswer: 2,
      explanation: "Monitors provide a more structured approach to synchronization compared to semaphores by bundling shared data and access methods into a single protected object."
    },
    {
      question: "A situation where two or more processes are waiting indefinitely for resources held by each other is called:",
      options: ["Starvation", "Blinking", "Deadlock", "Context Overlap"],
      correctAnswer: 2,
      explanation: "Deadlock is a 'circular wait' condition where no process can proceed because each is waiting for a resource held by another blocked process."
    }
  ];
  return (
...
signal(mutex);`}
        ...
        <Quiz
          title="Module 3.1 Quiz: Process Synchronization"
          questions={syncQuiz}
          subject="OS"
          unitId={3}
          moduleId={1}
