...
const FileSystemLayersAnimation: React.FC = () => {
  ...
  return (
    <div className="border-4 border-gray-400 bg-gray-50 p-4 md:p-8 rounded-2xl shadow-inner my-6">
      <div className="flex flex-col-reverse gap-3 max-w-md mx-auto">
        {layers.map((layer, i) => (
          <div key={i} className="relative group">
            <div className={`p-4 rounded-xl font-bold text-white text-center transition-all duration-300 shadow-md ${active === i ? `bg-${layer.color}-600 scale-105 ring-4 ring-offset-2 ring-${layer.color}-300` : `bg-${layer.color}-400 opacity-60`
              }`}>
              <span className="text-sm md:text-base uppercase tracking-tight">{layer.name}</span>
            </div>
            {active === i && (
              <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-2 h-8 bg-black rounded-full animate-pulse md:block hidden"></div>
            )}
            <div className="absolute -right-12 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-400 uppercase hidden md:block">
              Layer {6 - i}
            </div>
          </div>
        ))}
      ...
      <div className="mt-8 pt-4 border-t border-gray-200 text-center">
        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Active Processing Level</p>
        <p className={`text-xl font-black transition-colors duration-500 text-${layers[active].color}-600`}>
          {layers[active].name}
        </p>
      </div>
    </div>
...
const FileTypeAnimation: React.FC = () => {
  ...
  return (
    <div className={`border-4 bg-white p-6 md:p-12 rounded-2xl shadow-2xl my-8 transition-all duration-700 transform ${type === 0 ? 'border-blue-500' : type === 1 ? 'border-emerald-500' : type === 2 ? 'border-orange-500' : 'border-purple-500'
      }`}>
      <div className="flex flex-col items-center justify-center space-y-6">
        ...
        <div className="text-center">
          <h4 className={`text-3xl md:text-4xl font-black uppercase tracking-tighter transition-colors duration-500 ${type === 0 ? 'text-blue-600' : type === 1 ? 'text-emerald-600' : type === 2 ? 'text-orange-600' : 'text-purple-600'
            }`}>
            {current.name}
          </h4>
          <p className="text-sm font-bold text-gray-400 mt-2 italic px-4">
            {type === 0 ? 'General data stored as ASCII or binary.' : type === 1 ? 'Structures that contain references to other files.' : type === 2 ? 'Interface to I/O devices (terminals, console).' : 'Buffered access to hardware devices like disks.'}
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-3 mt-10">
        {types.map((t, idx) => (
          <div key={idx} className={`w-3 h-3 rounded-full transition-all duration-300 ${type === idx
            ? `${idx === 0 ? 'bg-blue-500 w-8' : idx === 1 ? 'bg-emerald-500 w-8' : idx === 2 ? 'bg-orange-500 w-8' : 'bg-purple-500 w-8'}`
            : 'bg-gray-200'
            }`}></div>
        ))}
      </div>
...
const Module5_2: React.FC = () => {
  const fileSystemQuiz = [
    {
      question: "Which layer of the File System maintains the 'File Control Blocks' (FCB) containing owner and size info?",
      options: ["I/O Control", "Basic File System", "Logical File System", "Application Programs"],
      correctAnswer: 2,
      explanation: "The Logical File System manages metadata (everything about the file except the actual content) via FCBs."
    },
    {
      question: "Which of the following is a modern file system used by Windows that supports encryption and permissions?",
      options: ["FAT", "NTFS", "ext4", "HFS"],
      correctAnswer: 1,
      explanation: "NTFS (New Technology File System) is the standard modern file system for Windows."
    },
    {
      question: "What makes 'Block Special Files' different from 'Character Special Files'?",
      options: ["They are only used on Windows.", "They provide buffered access and allow reading blocks of any size.", "They are used for terminals only.", "They don't store data."],
      correctAnswer: 1,
      explanation: "Block special files (like those for disks) use buffers to manage data in fixed-size chunks, unlike character special files which stream data byte-by-byte."
    },
    {
      question: "Which layer acts as the direct interface between hardware devices (like disks) and the OS software?",
      options: ["Application Programs", "I/O Control (Device Drivers)", "Directories", "Sequential Access"],
      correctAnswer: 1,
      explanation: "I/O Control contains the device drivers that translate logical requests into hardware commands."
    },
    {
      question: "What is a 'File Control Block' (FCB)?",
      options: ["A physical disk partition.", "A structure containing information about a file like owner, size, and location.", "A button on the keyboard.", "The power supply unit."],
      correctAnswer: 1,
      explanation: "The FCB stores all the important metadata required by the OS to manage and track a specific file."
    }
  ];
  return (
    <div className="module-content">
      ...
      <section className="content-section">
        ...
        <Quiz
          title="Module 5.2 Quiz: File Systems"
          questions={fileSystemQuiz}
          subject="OS"
          unitId={5}
          moduleId={2}
        />
