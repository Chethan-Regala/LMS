...
const Module4_9: React.FC = () => {
  const fragmentationQuiz = [
    {
      question: "What is the core problem that fragmentation causes in an Operating System?",
      options: ["It overclocks the CPU.", "It wastes memory by making free space unusable for new processes.", "It deletes files from the hard drive.", "It increases internet speed."],
      correctAnswer: 1,
      explanation: "Fragmentation occurs when memory is broken into many small, non-contiguous holes, making it impossible to fit a process that requires a certain amount of contiguous space."
    },
    {
      question: "Which of the following describes 'Internal Fragmentation'?",
      options: ["Memory wasted between two separate allocated blocks.", "Memory wasted inside a single allocated block because the process is smaller than the block.", "Memory wasted on the hard drive.", "Memory used by the cooling fan."],
      correctAnswer: 1,
      explanation: "Internal fragmentation happens most often in fixed partitioning, where a small process is put into a large predefined slot, leaving the remainder of that slot empty and unusable."
    },
    {
      question: "A system has three free blocks of 10 KB, 20 KB, and 15 KB. A process needs 35 KB. What is this an example of?",
      options: ["Internal Fragmentation", "External Fragmentation", "System Success", "Swapping"],
      correctAnswer: 1,
      explanation: "The total free memory (45 KB) is enough for the 35 KB process, but since the blocks are not connected (contiguous), the process cannot be loaded."
    },
    {
      question: "What is 'Compaction'?",
      options: ["Deleting the smallest processes to save space.", "Moving all occupied memory blocks to one side to create one large contiguous free block.", "Compressing images to save RAM.", "Turning off the monitor when not in use."],
      correctAnswer: 1,
      explanation: "Compaction is a 'shuffling' process that merges all existing free space into one large, useful block, effectively ending external fragmentation."
    },
    {
      question: "Which memory management scheme effectively eliminates external fragmentation without requiring compaction?",
      options: ["Dynamic Partitioning", "Fixed Partitioning", "Paging", "Manual Allocation"],
      correctAnswer: 2,
      explanation: "Because paging allows a process to be split into many small constant-sized pages, it can fit into any collection of free frames regardless of where they are in RAM."
    }
  ];
  return (
      ...
      <section className="content-section">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 md:p-8 my-6">
          <div className="space-y-4 max-w-lg mx-auto">
            <div className="flex items-center gap-3">
              <div className="w-20 text-xs font-bold text-gray-500">2 MB</div>
              <div className="flex-1 border-4 border-gray-400 bg-gray-100 h-10 rounded-xl flex items-center justify-center font-bold text-gray-400">EMPTY</div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-20 text-xs font-bold text-gray-500">4 MB</div>
              <div className="flex-1 flex flex-col sm:flex-row rounded-xl overflow-hidden border-4 border-blue-500 shadow-md">
                <div className="bg-blue-500 text-white px-4 py-2 font-bold text-center flex-1">P1 (3 MB)</div>
                <div className="bg-white text-red-500 px-4 py-2 text-[10px] font-black italic text-center animate-pulse">1 MB WASTED</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-20 text-xs font-bold text-gray-500">8 MB</div>
              <div className="flex-1 border-4 border-gray-400 bg-gray-100 h-10 rounded-xl flex items-center justify-center font-bold text-gray-400">EMPTY</div>
            </div>
          </div>
          <p className="text-center text-sm text-gray-600 mt-6 font-medium">Process P1 (3MB) allocated a 4MB block → 1MB is trapped inside and wasted (Internal Fragmentation).</p>
        </div>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 md:p-8 my-6">
          <div className="max-w-md mx-auto space-y-2">
            <div className="border-2 border-green-500 bg-green-50 p-2 rounded-lg font-bold text-gray-800 text-center text-xs">P1 (20 KB)</div>
            <div className="border-2 border-dashed border-red-400 bg-white p-2 rounded-lg font-bold text-red-500 text-center text-[10px]">HOLE (10 KB)</div>
            <div className="border-2 border-green-500 bg-green-50 p-2 rounded-lg font-bold text-gray-800 text-center text-xs">P2 (30 KB)</div>
            <div className="border-2 border-dashed border-red-400 bg-white p-2 rounded-lg font-bold text-red-500 text-center text-[10px]">HOLE (15 KB)</div>
            <div className="border-2 border-green-500 bg-green-50 p-2 rounded-lg font-bold text-gray-800 text-center text-xs">P3 (25 KB)</div>
            <div className="border-2 border-dashed border-red-400 bg-white p-2 rounded-lg font-bold text-red-500 text-center text-[10px]">HOLE (25 KB)</div>
          </div>
          <div className="mt-6 p-4 bg-white rounded-xl border-2 border-blue-200 text-center">
            <p className="text-sm font-bold text-blue-700">Total Free Space: 10 + 15 + 25 = 50 KB</p>
            <p className="text-xs text-red-600 font-bold mt-1">Cannot fit Process 5 (45 KB) because space is not CONTIGUOUS!</p>
          </div>
        </div>
        ...
        <Quiz
          title="Module 4.9 Quiz: Memory Fragmentation"
          questions={fragmentationQuiz}
          subject="OS"
          unitId={4}
          moduleId={9}
        />
