...
const Module2_3: React.FC = () => {
  const issuesQuiz = [
    {
      question: "In a Linux-based system, what occurs when a thread calls the fork() system call?",
      options: ["All threads in the process are duplicated.", "Only the thread that called fork() is duplicated in the child process.", "The process is terminated.", "The calling thread is replaced by a new program."],
      correctAnswer: 1,
      explanation: "In most Linux implementations, if one thread in a multithreaded process calls fork(), only that thread is duplicated in the child process."
    },
    {
      question: "Which system call replaces the current process's content with a new program?",
      options: ["fork()", "exec()", "wait()", "exit()"],
      correctAnswer: 1,
      explanation: "The exec() system call family replaces the current process image with a new program."
    },
    {
      question: "What is 'deferred cancellation' of a thread?",
      options: ["Terminating the thread immediately.", "The thread periodically checks if it should terminate, allowing it to clean up.", "Delaying thread creation.", "Suspending a thread indefinitely."],
      correctAnswer: 1,
      explanation: "Deferred cancellation allows a thread to reach a safe 'cancellation point' and clean up resources before terminating."
    },
    {
      question: "Which type of thread cancellation is compareable to 'suddenly pulling the plug'?",
      options: ["Deferred Cancellation", "Asynchronous Cancellation", "Synchronous Cancellation", "Voluntary Cancellation"],
      correctAnswer: 1,
      explanation: "Asynchronous cancellation stops the target thread immediately, which can lead to resource leaks or corrupted data."
    },
    {
      question: "What is a 'deadlock' in a multithreaded environment?",
      options: ["A thread running at maximum speed.", "Two or more threads waiting indefinitely for locks held by each other.", "A thread finish its task successfully.", "A thread being cancelled by the OS."],
      correctAnswer: 1,
      explanation: "A deadlock occurs when threads are blocked because each is waiting for a resource held by another, creating a circular wait."
    }
  ];
  return (
    <div className="module-content">
      ...
      <section className="content-section">
        ...
        <Quiz
          title="Module 2.3 Quiz: Threading Issues"
          questions={issuesQuiz}
          subject="OS"
          unitId={2}
          moduleId={3}
        />
